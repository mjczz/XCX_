...
class UserToken extends Token {
    ...
    public function get()
    {
        ...
        if(empty($wxResult)){
            ...
            // 这种情况通常是由于传入不合法的code
            // 这个是服务器内部异常，要记录日志，所以没有用自定义异常，不需要返回到客户端(在构建基本异常的时候是这么构建的)
            throw new Exception('获取session_key及openID时异常，微信内部错误');
        } else{
            // 建议用明确的变量来表示是否成功
            // 微信服务器并不会将错误标记为400，无论成功还是失败都标记成200(在rest 里200代表成功)
            // 这样非常不好判断，只能使用errcode是否存在来判断
            $loginFail = array_key_exists('errcode', $wxResult);
            if($loginFail){
                // 封装成方法后，以后如果在方法里面记录日志、发送邮件等好扩展一些
    ...
    private function processLoginError($wxResult)
    {
        throw new WeChatException(
            ['msg' => $wxResult['errmsg'], 'errorCode' => $wxResult['errcode']]);
    }
    ...
    private function saveToCache($wxResult)
    {
        ...
        if(!$result){
            throw new TokenException(
                ['msg' => '服务器缓存异常', 'errorCode' => 10005]);
        }
        return $key;
    }

    /**
     * 拿到openid，数据库里看一下，这个openid是否已经存在；存在则不处理，不存在那么就新增一条user记录；生成令牌，准备缓存数据，写入缓存，把令牌返回到客户端去
     * key:令牌
    ...
    private function grantToken($wxResult)
    {
        ...
        $user = User::getByOpenID($openid);// 根据openid获取用户信息
        if(!$user)
            // 借助微信的openid作为用户标识
